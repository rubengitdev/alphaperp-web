    ...
    <div className="terminal-layout">
      ...
      <div className="terminal-panel terminal-center-top">
        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid rgba(255, 255, 255, 0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <span style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#F7F4ED' }}>{headerAsset ? headerAsset.symbol : 'Select Asset'}</span>
            <span style={{ fontSize: '0.875rem', color: '#A3A3A3', marginLeft: '0.5rem' }}>{headerAsset ? 'Perpetual Options' : ''}</span>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#F7F4ED' }}>{headerAsset && headerAsset.price > 0 ? `$${headerAsset.price.toLocaleString(undefined, { minimumFractionDigits: 2 })}` : '-'}</div>
            <div style={{ fontSize: '0.875rem', color: headerAsset && headerAsset.change24h >= 0 ? '#20D9C5' : '#F87171' }}>
      ...
      <div className="terminal-panel terminal-center-bottom" style={{ padding: '1rem', position: 'relative' }}>
        <div style={{ 
          ...
          <button 
            ...
            style={{ padding: '0.75rem 1rem', backgroundColor: 'transparent', color: bottomTab === 'chain' ? '#20D9C5' : '#A3A3A3', border: 'none', borderBottom: bottomTab === 'chain' ? '2px solid #20D9C5' : '2px solid transparent', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 'bold', letterSpacing: '0.1em' }}
          >
            OPTION CHAIN
          </button>
          <button 
            onClick={() => setBottomTab('positions')}
            style={{ padding: '0.75rem 1rem', backgroundColor: 'transparent', color: bottomTab === 'positions' ? '#20D9C5' : '#A3A3A3', border: 'none', borderBottom: bottomTab === 'positions' ? '2px solid #20D9C5' : '2px solid transparent', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 'bold', letterSpacing: '0.1em' }}
          >
            OPEN POSITIONS
          </button>
        </div>
      ...
      <div className="terminal-panel terminal-right">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', padding: '1rem', borderBottom: '1px solid rgba(255, 255, 255, 0.05)' }}>
          <button 
            ...
            style={{ padding: '0.75rem', backgroundColor: orderAction === 'buy' && orderType === 'call' ? 'rgba(32, 217, 197, 0.1)' : 'transparent', color: orderAction === 'buy' && orderType === 'call' ? '#20D9C5' : '#A3A3A3', border: orderAction === 'buy' && orderType === 'call' ? '1px solid #20D9C5' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 'bold', transition: 'all 0.2s' }}
          >
            Buy Call
          </button>
          <button 
            onClick={() => { setOrderAction('buy'); setOrderType('put'); }}
            style={{ padding: '0.75rem', backgroundColor: orderAction === 'buy' && orderType === 'put' ? 'rgba(32, 217, 197, 0.1)' : 'transparent', color: orderAction === 'buy' && orderType === 'put' ? '#20D9C5' : '#A3A3A3', border: orderAction === 'buy' && orderType === 'put' ? '1px solid #20D9C5' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 'bold', transition: 'all 0.2s' }}
          >
            Buy Put
          </button>
          <button 
            onClick={() => { setOrderAction('sell'); setOrderType('call'); }}
            style={{ padding: '0.75rem', backgroundColor: orderAction === 'sell' && orderType === 'call' ? 'rgba(32, 217, 197, 0.1)' : 'transparent', color: orderAction === 'sell' && orderType === 'call' ? '#20D9C5' : '#A3A3A3', border: orderAction === 'sell' && orderType === 'call' ? '1px solid #20D9C5' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 'bold', transition: 'all 0.2s' }}
          >
            Sell Call
          </button>
          <button 
            onClick={() => { setOrderAction('sell'); setOrderType('put'); }}
            style={{ padding: '0.75rem', backgroundColor: orderAction === 'sell' && orderType === 'put' ? 'rgba(32, 217, 197, 0.1)' : 'transparent', color: orderAction === 'sell' && orderType === 'put' ? '#20D9C5' : '#A3A3A3', border: orderAction === 'sell' && orderType === 'put' ? '1px solid #20D9C5' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 'bold', transition: 'all 0.2s' }}
          >
            Sell Put
          </button>
        </div>
